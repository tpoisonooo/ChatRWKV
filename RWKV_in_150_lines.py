...
class TimeMixer(torch.nn.Module):
    def __init__(self, time_mix_k, time_mix_v, time_mix_r, time_first, time_decay, kw, vw, rw, ow, ln_weight, ln_bias):
        super().__init__()
...
class ChannelMixer(torch.nn.Module):
    ...
    def forward(self, x, state, i):
        y = x
        x = F.layer_norm(x, (1024, ), weight=self.ln_weight, bias=self.ln_bias)
        xk = x * self.time_mix_k + state[5 * i + 0].flatten() * (1 - self.time_mix_k)
        xr = x * self.time_mix_r + state[5 * i + 0].flatten() * (1 - self.time_mix_r)
        state[5 * i + 0] = x
        r = torch.sigmoid(self.rw @ xr)
        k = torch.square(torch.relu(self.kw @ xk))  # square relu, primer paper
        return y + (r * (self.vw @ k))
    ...
    def forward(self, tokenid, state):
        with torch.no_grad():
            ...
            for i in range(self.args.n_layer):
                itensor = torch.full([1], i, dtype=torch.int32)
                ...
                channel_mixer = ChannelMixer(ffn.time_mix_k, ffn.time_mix_r, ffn.key.weight, ffn.value.weight, ffn.receptance.weight, ln2.weight, ln2.bias)
                x = channel_mixer.forward(x, state, itensor)
...
# @torch.jit.script_method
# def channel_mixing_export(self, x, state, i: int, time_mix_k, time_mix_r, kw, vw, rw, ln_weight, ln_bias):
#     x = F.layer_norm(x, (1024, ), weight=ln_weight, bias=ln_bias)
...
# @torch.jit.script_method
# def time_mixing_export(self, x, state, i: int, time_mix_k, time_mix_v, time_mix_r, time_first, time_decay, kw, vw, rw, ow, ln_weight, ln_bias):
#     x = F.layer_norm(x, (1024, ), weight=ln_weight, bias=ln_bias)
